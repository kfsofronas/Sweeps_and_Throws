...
public class Player2Controller : MonoBehaviour
{
	...
	void Start()
	{
		...
		still = true;
		movement_fwd = new Vector3(-1 * speed, 0, 0);
		movement_back = new Vector3(speed, 0, 0);
		...
		techPushbackVector = new Vector3(THROWTECHPUSHBACK, 0, 0);
		sweepPushbackVector = new Vector3(SWEEPPUSHBACK, 0, 0);
	...
	void Update()
	{
		...
		if (!gameOver)
		{
			...
			if (controllable)
			{
				if (Input.GetAxisRaw("P2_Horizontal") < MINIMUM_TILT * -1) { forward = true; still = false; backward = false; }
				else if (Input.GetAxisRaw("P2_Horizontal") > MINIMUM_TILT) { backward = true; still = false; forward = false; }
				else { still = true; forward = false; backward = false; }

				if (Input.GetAxisRaw("P2_Sweep") > MINIMUM_TILT) { sweep = true; } else { sweep = false; }
				if (Input.GetAxisRaw("P2_Throw") > MINIMUM_TILT) { throwing = true; } else { throwing = false; }
			}
			else
			{
			...
			if (Input.GetAxisRaw("P2_Vertical") < MINIMUM_TILT * -1) { crouch = true; }
			else { crouch = false; }
			...
			if (anim.GetCurrentAnimatorStateInfo(0).IsName("Player_HitThrow") && spriteRender.sprite.name.Equals("hit_throw1") && !anim1done) { newPos.x = player1Transform.position.x + throwAnim1; trans.position = newPos; anim1done = true; print("did anim1"); print("anim1done = " + anim1done.ToString()); }
			if (anim.GetCurrentAnimatorStateInfo(0).IsName("Player_HitThrow") && spriteRender.sprite.name.Equals("hit_throw2flipped") && !anim2done) { newPos.x = player1Transform.position.x + throwAnim2; trans.position = newPos; anim2done = true; }
			if (anim.GetCurrentAnimatorStateInfo(0).IsName("Player_HitThrow") && spriteRender.sprite.name.Equals("hit_sweep2") && !anim3done) { newPos.x = player1Transform.position.x + throwAnim3; trans.position = newPos; anim3done = true; }
			...
			if (spriteRender.sprite.name.Equals("hit_sweep2") && !healthDone)
			{
				...
				print("did health");
				healthDone = true;
				print("healthdone = " + healthDone.ToString());
			}
			...
			*/
			...
			if (GameObject.FindGameObjectWithTag("Player1").GetComponent<PlayerController>().health == 0) { gameOver = true; win = true; trans.parent.GetChild(0).gameObject.SetActive(true); }
		}
	...
	void Animate()
	{
		...
		GameObject.FindGameObjectWithTag("P2Health").GetComponent<Animator>().SetInteger("health", health);
